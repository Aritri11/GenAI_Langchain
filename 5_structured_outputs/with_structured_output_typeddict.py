...
#     summary: Annotated[str,'A brief summary of the review'] #Annotated is used to guide the llm such that it dosen't confuse the actual meaning of the words
...
class Review(TypedDict):
    ...
    summary: Annotated[str,'A brief summary of the review'] #Annotated is used to guide the llm such that it dosen't confuse the actual meaning of the words
    sentiment: Annotated[Literal["pos", "neg"],'Return sentiment of the review either negative, positive or neutral'] #literal when used makes only decisions based on the ones given inside the literal
    pros: Annotated[Optional[list[str]],'write down all the pros inside a list'] #optional is for the llm to understand that if not explicitly mentioned do not give that
